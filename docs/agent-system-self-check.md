...
| 能力 | 现状 | 优先级 | 备注 |
|------|------|--------|------|
| 可观测性 / Trace | ✅ 已实现（2026-08-20）：`Debouncer` 窗口触发时生成 trace_id，随 `SessionState` checkpoint 落盘并贯穿 debounce→main_agent→execute→投递；confirm-race resume 沿用原 id 不重开。与 `execution_id`（幂等身份）并存不合并 | 已闭环 | 下一步是趁着有了统一 id，去补 debounce+graph 的并发场景测试（见下一行） |
