...
- Admin 观测台改版为 session 式（用户反馈"原始"后重设计，见 DECISION.md 2026-08-21 session 条目）：左侧 sidebar 每用户一个 session（中文渠道徽标 飞书/评估/测试、最近活跃、失败计数、近 7 天成本），详情页=档案 chips+摘要折叠+成本小计+对话回放时间轴（IM 气泡+运维元数据轨，失败红点红描边，execution 系统条）；总成本与 Eval 保留为全局视图（图表几何原样迁移经 review 证实未重写）。新增 `list_sessions` 聚合与 `GET /api/sessions`，既有四端点不动；smoke 断言逐条迁移无静默删除、新渲染路径 XSS 经 review 逐点核（未知渠道原样展示是新注入面已覆盖）。2026-08-21，`pytest` 414 passed（398 → 414）。
- Admin 观测台 V1 上线（纯只读，见 DECISION.md 2026-08-21）：FastAPI（optional extra `admin`）+ 单文件前端，127.0.0.1 无鉴权，四板块——回合流（user 筛选/limit）、成本仪表（复用 report_costs 聚合与 5 色规范）、记忆与摘要（档案+conversation_summary+pending 计数；checkpoint 经只读 SQLite 连接读取，发现并绕开 SqliteSaver.get_tuple 内部 setup() 写库的坑——见 PITFALLS）、eval 报告趋势（passed 率/clarity 折线+逐样本详情）。安全：全只读无写端点、eval 报告名白名单正则防穿越、XSS 全插入点转义（review 逐点核）、无外部资源。真实启动验收四端点通车。2026-08-21，`pytest` 393 passed（341 → 393）。
- 成本埋点与报表上线（只记账不限流，enforcement 挂对外前置项，见 DECISION.md 2026-08-21）：新增 `var/logs/model_calls.jsonl` 结构化记账（provider/purpose/用户归属/prompt+completion tokens/耗时），DeepSeek 三类调用在 `_json_completion` 单点收口（记账在 JSON 解析前——token 烧了就算数，有回归测试钉住），summarizer 经 owner 参数归属用户；`scripts/report_costs.py` 输出终端汇总 + 单文件 HTML 报表（按日×purpose 堆叠图，5 色过 CVD 校验）。金额按官方定价页核实的 v4-flash 高峰无缓存美元价做保守上界估算（deepseek-chat 别名映射官方未确认，对账以控制台账单为准）。已知缺口：summarizer is_error 分支不记账（披露接受）。turns.jsonl 的测试污染已随后用同一 conftest 隔离模式治理（341 passed 后真实文件零增行实测）。2026-08-21，`pytest` 341 passed（315 → 341）。
- Guardrails 泄漏防护三层上线（刻意不建输入检测器，见 DECISION.md 2026-08-21）：decide prompt 加不复述内部设定条款；graph 出口确定性 guard（9 个强特异性特征串命中即整条替换固定话术+WARNING，作用于 decide 话术与 finalize 返回值，接线顺序保证盖住 `_remove_unverified_memory_claim` 的 instruction 回拼路径）；eval inject-002 断言收紧 + 新增换措辞变体 inject-007。验收回归 23/23 PASSED，实证层次正确：prompt 加固单独生效（模型对直接索要与换措辞诱导都礼貌拒绝，inject-002 judge clarity 1→4），出口 guard 零触发作为安全网待命。误报方向经 12 条正常话术反例锁定。2026-08-21，`pytest` 315 passed（290 → 315）。
